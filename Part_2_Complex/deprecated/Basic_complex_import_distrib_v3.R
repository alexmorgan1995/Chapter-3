...
amrimp <- function(t, y, parms) {
  with(as.list(c(y, parms)), {
    ...
    dIrhA4 = (1-psi)*(imp4)*(1-alpha)*(betaHA*fracimp1*propres_imp4*Sh) - rh*IrhA4 - uh*IrhA4  
...
#Skewed
base_skew_fracimp <- data.frame(matrix(rbeta(1000*10, shape1 =  0.5, shape2 =  2), 
                                  nrow = 1000, ncol = 10))
...
explore_parms <- list(homo_fracimp, skew_fracimp)
...
for(i in 1:2) {
  ...
  plot_imp[[i]] <- p_import_comp
}


p_dist <- ggarrange(plot_imp[[1]], plot_imp[[2]], nrow = 1,ncol = 2, labels = c("A", "B"), font.label = c(size = 20))

ggsave(p_dist, filename = "dist_uncert_sample.png", dpi = 300, type = "cairo", width = 11, height = 6, units = "in",
       path = "//csce.datastore.ed.ac.uk/csce/biology/users/s1678248/PhD/Chapter_3/Models/Chapter-3/Figures")

# Plotting Distributions --------------------------------------------------

dist_plots <- data.frame("Uniform" = dbeta(seq(0, 1, by = 0.01), 1, 1),
                         "Skewed" = dbeta(seq(0, 1, by = 0.01), 0.5, 2),
                         "x" = seq(0, 1, by = 0.01))

dist_plots <- melt(dist_plots, measure.vars = c("Uniform", "Skewed"), id.vars = c("x"))

dist <- ggplot(dist_plots, aes(x=x,y = value, col = variable)) + geom_line(size = 1.2) + theme_bw() + 
  theme(legend.position= "bottom", legend.text=element_text(size=12), legend.title =element_text(size=12), axis.text=element_text(size=12), 
        axis.title.y=element_text(size=12), axis.title.x= element_text(size=12), plot.margin = unit(c(0.35,1,0.35,1), "cm"),
        legend.spacing.x = unit(0.3, 'cm')) +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) + 
  labs(x = "Probability", 
       y = "Density") +
  scale_color_manual(labels  = c(expression("Beta"*"("*alpha*"="*1*", "*beta*"="~1*")"),
                                 expression("Beta"*"("*alpha*"="*0.5*", "*beta*"="~2*")")),
                     values = c("darkblue", "red"))

ggsave(dist, filename = "dist_example.png", dpi = 300, type = "cairo", width = 6, height = 6, units = "in",
       path = "//csce.datastore.ed.ac.uk/csce/biology/users/s1678248/PhD/Chapter_3/Models/Chapter-3/Figures")
