...
MAP_parms <- data.frame("Parameter" = colnames(post_amp), "mean" = colMeans(post_amp))
...
#Average Foodborne Disease Imports

weight_mean_fbd <- sum(c(country_data_imp[-1,"Foodborne_Carriage_2019"] * as.numeric(country_data_imp[-1,"Normalised_Usage_2018"]), 
                         MAP_parms[MAP_parms$Parameter == "imp_nEU",2]* (1-sum(as.numeric(country_data_imp[-1,"Normalised_Usage_2018"]))))) / 10
mean_fbd <- mean(c(country_data_imp[-1,"Foodborne_Carriage_2019"], MAP_parms[MAP_parms$Parameter == "imp_nEU",2]))

#Baseline Foodborne disease Animals (supposed to be 0.017173052)
model_fbd <- sum(tail(out[,3:4],1))

#Average resistance in imports
weight_mean_res <- sum(c(country_data_imp[-1,"Prop_Amp_Res"] * as.numeric(country_data_imp[-1,"Normalised_Usage_2018"]), 
                         MAP_parms[MAP_parms$Parameter == "propres_impnEU",2]* (1-sum(as.numeric(country_data_imp[-1,"Normalised_Usage_2018"]))))) / 10
mean_res <- mean(c(country_data_imp[-1,"Prop_Amp_Res"], MAP_parms[MAP_parms$Parameter == "propres_impnEU",2]))

#Baseline Resistance Domestic (supposed to 0.11111)
model_res <- tail(out[,4],1)/sum(tail(out[,3:4],1))
