...
parms2 <- c(ra = 60^-1, rh = (5.5^-1), ua = 240^-1, uh = 28835^-1, psi = UK_food_usage,
            ...
            betaAA = 0.014400803, betaHD = 0.00128034, betaHI_EU = 0.000538055, imp_nEU = 0.238599735, propres_impnEU = 0.608446385, 
            phi = 0.015865365, kappa = 9.509298715, alpha =  0.476839491, zeta = 0.006354449, eta = 0.11016,
            tau = UK_amp_usage)
...
singlerun <- function(x, G, init.state, distanceABC, fitmodel, thetaparm, epsilon, 
  ...
  while(i <= 1) {
    ...
    if(prior.non.zero(new.parms, lm.low, lm.upp)) {
      ...
      thetaparm[c("betaAA", "phi", "kappa", "alpha", "zeta", "betaHD", "betaHI_EU", "imp_nEU", "propres_impnEU")] <- new.parms
...
ABC_algorithm <- function(N, G, distanceABC, fitmodel, tau_range, init.state, data, epsilon, lm.low, lm.upp, thetaparm)  {
  ...
  for(g in 1:G) {
    ...
    colnames(res.old) <- c("betaAA", "phi", "kappa", "alpha", "zeta", "betaHD", "betaHI_EU", "imp_nEU", "propres_impnEU")
    write.csv(res.old, file = paste("C:/Users/amorg/Documents/PhD/Chapter_3/Models/Chapter-3/Model_Fit_Data/Part2/ABC_post_complex_amppigs_TEST",g,".csv",sep=""), row.names=FALSE)
  }
  return(out)
}
...
test <- ABC_algorithm(N = 1,
                      ...
                      lm.upp = c(0.05, 0.1, 30, 1, 0.02, 0.01, 0.001, 0.3, 1), 
                      thetaparm = c(ra = 60^-1, rh = (5.5^-1), ua = 240^-1, uh = 28835^-1, psi = 0.656,
                                    ...
                                    imp1 = country_data_imp[2,"FBD_gen"], imp2 = country_data_imp[3,"FBD_gen"], imp3 = country_data_imp[4,"FBD_gen"], imp4 = country_data_imp[5,"FBD_gen"], 
                                    imp5 = country_data_imp[6,"FBD_gen"], imp6 = country_data_imp[7,"FBD_gen"], imp7 = country_data_imp[8,"FBD_gen"],
                                    imp8 = country_data_imp[9,"FBD_gen"], imp9 = country_data_imp[10,"FBD_gen"],
