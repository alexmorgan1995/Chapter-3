...
# Model for Attributable Incidence ----------------------------------------

amrimp_inc <- function(t, y, parms) {
  with(as.list(c(y, parms)), {
...
amp_post <- do.call(rbind,
                    lapply(list.files(path = "//csce.datastore.ed.ac.uk/csce/biology/users/s1678248/PhD/Chapter_3/Models/Chapter-3/Model_Fit_Data/Part2/betaha/inch", pattern = "complex"), read.csv))

amp_post$gen <- as.vector(sapply(1:(nrow(amp_post)/nrow(read.csv(tail(list.files(path = "//csce.datastore.ed.ac.uk/csce/biology/users/s1678248/PhD/Chapter_3/Models/Chapter-3/Model_Fit_Data/Part2/betaha/inch", pattern = "complex"), 1)))), 
                                 function(x) rep(paste0("gen_",x), nrow(read.csv(tail(list.files(path = "//csce.datastore.ed.ac.uk/csce/biology/users/s1678248/PhD/Chapter_3/Models/Chapter-3/Model_Fit_Data/Part2/betaha/inch", pattern = "complex"), 1))))))
...
# Test Model Fit and Generate HDIs ----------------------------------------
...
parmtau <- seq(0,0.055, by = 0.004)

output1 <- data.frame(matrix(nrow = length(parmtau), ncol = 8))
output_ribbon <- data.frame(matrix(nrow = 0, ncol = 3))
fitparms <- c("betaAA", "phi", "kappa", "alpha", "zeta", "betaHA", "imp_nEU", "propres_impnEU")

for (i in 1:length(parmtau)) {
  
  temp_ribbon <- data.frame(matrix(NA, nrow = 1000, ncol = 3))
  
  thetaparm1 <- thetaparm
  thetaparm1[fitparms] = c(MAP_parms["betaAA",2], MAP_parms["phi",2], MAP_parms["kappa",2], MAP_parms["alpha",2],
                           MAP_parms["zeta",2], MAP_parms["betaHA",2], MAP_parms["imp_nEU",2], MAP_parms["propres_impnEU",2])
  thetaparm1["tau"] <- parmtau[i]
  
  out <- runsteady(y = init, func = amrimp, times = c(0, Inf), parms = thetaparm1)
  
  output1[i,1] <- parmtau[i]
  output1[i,2] <- (out[[2]]*(446000000))/100000
  output1[i,3] <- (out[[3]]*(446000000))/100000
  output1[i,4] <- ((out[[2]] + out[[3]])*(446000000))/100000
  output1[i,5] <- out[[3]] / (out[[2]] + out[[3]])
  output1[i,6] <- out$y["Ira"] / (out$y["Isa"] + out$y["Ira"])
  output1[i,7] <- out$y["Ish"]
  output1[i,8] <- out$y["Irh"]
  
  for(z in 1:1000) {
    
    thetaparm1[fitparms] = c(post_amp[z,"betaAA"], post_amp[z,"phi"], post_amp[z,"kappa"], post_amp[z,"alpha"],
                             post_amp[z,"zeta"], post_amp[z,"betaHA"], post_amp[z,"imp_nEU"], post_amp[z,"propres_impnEU"])
    ...
    print(paste0("tau: ", temp_ribbon[z,1], ", ", (z/1000)*100, "%"))
    
  }
  output_ribbon <- rbind.data.frame(output_ribbon, temp_ribbon)
}

colnames(output1)[1:8] <- c("tau","InfHumans","ResInfHumans","ICombH","IResRat","IResRatA","PrevHS","PrevHR")
colnames(output_ribbon)[1:3] <- c("tau","particle","IResRatA")
...
for(i in 1:length(parmtau)) {
  HDI_ribbon <- rbind(HDI_ribbon, 
                      data.frame("tau" = unique(output_ribbon$tau)[i],
                                 "lowHDI" = hdi(output_ribbon$IResRatA[output_ribbon$tau == unique(output_ribbon$tau)[i]], credMass = 0.95)[[2]],
                                 "highHDI" = hdi(output_ribbon$IResRatA[output_ribbon$tau == unique(output_ribbon$tau)[i]], credMass = 0.95)[[3]]
                      ))
}
...
p_rib_amp_pig <- ggplot(melt_amp_pigs, aes(x = Usage, y= ResPropAnim, color = Country))  + geom_point() + theme_bw() + 
  ...
  geom_line(data = output1, aes(x = tau, y= IResRatA), col = "red", size = 1.1) +
...
# Generate Attributable Resistance  -------------------------------------------------------
