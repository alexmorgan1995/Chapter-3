...
amp_post$gen <- as.vector(sapply(1:2, function(x) rep(paste0("gen_",x), 1000)))
tet_post <- do.call(rbind,
                    lapply(list.files(path = "C:/Users/amorg/Documents/PhD/Chapter_3/Models/fit_data", pattern = "^complexmodel_ABC_SMC_gen_tet.*?\\.csv"), read.csv))
tet_post$gen <- as.vector(sapply(1:2, function(x) rep(paste0("gen_",x), 1000)))

# Assess Posterior --------------------------------------------------------

l_amp_post <- lapply(1:length((colnames(amp_post))[-1]), function(x) melt(tet_post, id.vars = "gen", measure.vars = colnames(amp_post)[x])[,c(1,3)])
l_tet_post <- lapply(1:length((colnames(tet_post))[-1]), function(x) melt(tet_post, id.vars = "gen", measure.vars = colnames(tet_post)[x])[,c(1,3)])
...
tet_p_list <- list()
for(i in 1:length(names)){ 
  amp_p_list[[i]] <- ggplot(l_tet_post[[i]], aes(x=value, fill= gen)) + geom_density(alpha=.5) + 
    scale_x_continuous(expand = c(0, 0), name = names[i]) + 
    scale_y_continuous(expand = c(0, 0), name = "") +
    labs(fill = NULL) + scale_fill_discrete(labels = sapply(1:length(unique(tet_post$gen)), function(x) paste0("Generation ", x)))+
    theme(legend.text=element_text(size=14),  axis.text=element_text(size=14),
          axis.title.y=element_text(size=14),axis.title.x= element_text(size=14), plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"))
}
...
pairs_tet <- GGally::ggpairs(tet_post[-11], lower=list(continuous=my_fn)) + theme_bw()
pairs_amp <- GGally::ggpairs(amp_post[-11], lower=list(continuous=my_fn)) + theme_bw()
...
#MAP_parms <- as.data.frame(map_estimate(tet_post[tet_post$gen == tail(unique(tet_post$gen), 1),][-11]))
MAP_parms <- data.frame("parms" = colnames(tet_post)[-11], "mean" = colMeans(tet_post[tet_post$gen == tail(unique(tet_post$gen), 1),][-11]))
...
#Ranges for Tetracycline Fit 

last_gen_tet <- lapply(1:(ncol(tet_post)-1), function(x) range(tet_post[colnames(tet_post)[x]]))
tet_runif <- lapply(1:(ncol(tet_post)-1), function(x) runif(1000, last_gen_tet[[x]][[1]], last_gen_tet[[x]][[2]]))
names(tet_runif) <- colnames(tet_post)[-11]
...
for(j in 1:length(names(tet_runif))){
  tet_uncert[[j]] <- local ({
    
    usage_frame_1 <- data.frame(matrix(nrow = 0, ncol = 2))
    
    for(x in 1:length(tet_runif[[j]])) {
      dump_data <- vector() 
      parms2 <- parms
      parms2[names(tet_runif)[x]] <- tet_runif[[j]][[i]]
      ...
      dump_data[2] <- paste0("Parm_Set_", names(tet_runif)[j])
      usage_frame_1 <- rbind(usage_frame_1, dump_data)
      
      print(paste0("Parameter Set ", names(tet_runif)[j]," - ", round(x/length(tet_runif[[j]]), digits = 4)*100, "%"))
    }
...
comb_data <- do.call("rbind", tet_uncert)
...
parm_viol_plot <- ggplot(comb_data, aes(x=Parameter, y=as.numeric(relRes), fill=Parameter)) + theme_bw() +
  ...
  scale_x_discrete(labels=c(names(tet_runif))) + 
...
ggplot(usage_frame, aes(x = domusage, y = relchange)) + geom_line()
