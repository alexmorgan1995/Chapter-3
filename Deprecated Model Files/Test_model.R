...
# Take a Look at Beta_Imp and Beta_nEU ------------------------------------
...
for(i in 1:21) {
  ...
  amp_cont_data <- rbind(amp_cont_data, tempcut)
  colnames(amp_cont_data) <- grep("fracimp", names(parms), value=TRUE)
}

data_list_perc <- list(amp_cont_data, amp_res_data)
...
colnames(cont_minmax_data) <- sapply(seq(0, 20, by = 1), function(x) paste0(x, "%"))
cont_minmax_data$base <- data_base[,1];  cont_minmax_data$usage <- usage_threshold
plot_cont <- melt(cont_minmax_data, measure.vars = (colnames(cont_minmax_data))[-23], id.vars = c("usage"))

res_minmax_data <- as.data.frame(cbind(sapply(data_imp_list[[3]], "[[", 1)))
colnames(res_minmax_data) <- sapply(0:10, function(x) paste0(x,"0%"));  res_minmax_data$usage <- usage_threshold
plot_res <- melt(res_minmax_data, measure.vars = (colnames(res_minmax_data))[-12], id.vars = c("usage"))

# Plotting ----------------------------------------------------------------

ggplot(plot_cont, aes(x = usage, y = value, col = variable, size = variable)) + geom_line() +
  scale_color_manual(values = c(palette(rainbow(21)), "black")) + 
  scale_size_manual(values = c(rep(1, 21), 3)) + theme_bw() + 
...
ggplot(plot_res, aes(x = usage, y = value, col = variable, size = variable)) + geom_line() +
  scale_color_manual(values = c(palette(rainbow(11)), "black")) + 
  scale_size_manual(values = c(rep(1, 11), 3)) + theme_bw() + 
  labs(x ="Proportion of Food From Domestic Origins", y = "Relative Reduction in the Efficacy of Livestock Antibiotic Curtailment on Resistance") +
  theme(legend.position= "bottom", legend.text=element_text(size=12), legend.title =element_text(size=12), axis.text=element_text(size=12), 
        axis.title.y=element_text(size=12), axis.title.x= element_text(size=12), plot.margin = unit(c(0.35,1,0.35,1), "cm"),
        legend.spacing.x = unit(0.3, 'cm'))

mean(country_data_imp$Foodborne_Carriage_2019)
