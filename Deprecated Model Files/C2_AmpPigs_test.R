...
ABC_algorithm <- function(N, G, sum.stats, distanceABC, fitmodel, tau_range, init.state, data)  {
  N_ITER_list <- list()
  fit_parms <- c("betaAA", "phi", "kappa", "alpha", "zeta", "betaHD", "betaHI")
  ...
  for(g in 1:G) {
    ...
    while(i <= N) {
      ...
      if(prior.non.zero(c(d_betaAA, d_phi, d_kappa, d_alpha, d_zeta, d_betaHD, d_betaHI))) {
        m <- 0
        thetaparm <- c(ra = 60^-1, rh = (5.5^-1), ua = 240^-1, uh = 28835^-1, psi = UK_food_usage, betaAA = d_betaAA,  
                       phi = d_phi, kappa = d_kappa, alpha = d_alpha, zeta = d_zeta, betaHD = d_betaHD, betaHI = d_betaHI, 
                       fracimp = EU_cont, propres_imp = EU_res, eta = 0.11016)
        ...
        if((dist_mod[1] <= epsilon[["dist"]][g]) && (dist_mod[2] <= epsilon[["foodH"]][g]) && (dist_mod[3] <= epsilon[["AMRH"]][g]) && 
          ...
          res.new[i,]<- c(d_betaAA, d_phi, d_kappa, d_alpha, d_zeta, d_betaHD, d_betaHI)  
    ...
    colnames(res.new) <- c("betaAA", "phi", "kappa", "alpha", "zeta", "betaHD", "betaHI")
    write.csv(res.new, file = paste("C:/Users/amorg/Documents/PhD/Chapter_3/Models/Chapter-3/Part_1_Basic/ABC_post_amppigs_",g,".csv",sep=""), row.names=FALSE)
  }
  return(N_ITER_list)
}
...
lm.upp = c(0.05, 0.05, 50, 1, 0.01, 0.5, 0.01) #Upper and lower bounds for the priors - for the multivariate normal dist pert kernel
...
epsilon = list("dist" =  c(5 , 5, 5, 5, 5, 5, 5, 5),
               ...
               "AMRH" =  c(UK_hum_ampres, UK_hum_ampres*0.8, UK_hum_ampres*0.6, UK_hum_ampres*0.4, UK_hum_ampres*0.3, UK_hum_ampres*0.2, UK_hum_ampres*0.15, UK_hum_ampres*0.1),
               "foodA" = c(UK_cont, UK_cont*0.8, UK_cont*0.6, UK_cont*0.4, UK_cont*0.3, UK_cont*0.2, UK_cont*0.15, UK_cont*0.1),
               "AMRA" =  c(UK_amp_res, UK_amp_res*0.8, UK_amp_res*0.6, UK_amp_res*0.4, UK_amp_res*0.3, UK_amp_res*0.2, UK_amp_res*0.15, UK_amp_res*0.1))
