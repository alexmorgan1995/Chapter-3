...
amrimp <- function(t, y, parms) {
  with(as.list(c(y, parms)), {
    ...
    dIsa = betaAA*Isa*Sa + phi*Ira - kappa*tau*Isa - tau*Isa - ra*Isa - ua*Isa
    dIra = (1-alpha)*betaAA*Ira*Sa + tau*Isa - phi*Ira - ra*Ira - ua*Ira 
...
computeDistanceABC_ALEX <- function(sum.stats, distanceABC, fitmodel, tau_range, thetaparm, init.state, times, data) {
  ...
  for (i in 1:length(tau_range)) {
    ...
    parms2 = c(ra = thetaparm[["ra"]], rh = thetaparm[["rh"]], ua = thetaparm[["ua"]], uh = thetaparm[["uh"]], tau = tau_range[i], psi = thetaparm[["psi"]],
               ...
               phi = thetaparm[["phi"]], kappa = thetaparm[["kappa"]], alpha = thetaparm[["alpha"]])
...
ABC_algorithm <- function(N, G, sum.stats, distanceABC, fitmodel, tau_range, init.state, times, data, data_match) {
  ...
  for(g in 1:G) {
    i <- 1
    dist_data <- data.frame(matrix(nrow = 100, ncol = 5))
    N_ITER <- 1
    ...
    while(i <= N) {
      ...
      if(g==1) {
        ...
        d_betaHH <- runif(1, 0, 0.1)
        d_betaHI_EU <- runif(1, 0, 0.0025)
      ...
      if(prior.non.zero(c(d_betaAA, d_phi, d_kappa, d_alpha, d_betaHD, d_betaHH, d_betaHI_EU, d_imp_nEU, d_propres_impnEU))) {
        m <- 0
        ...
        thetaparm <- c(ra = 60^-1, rh = (5.5^-1), ua = 240^-1, uh = 28835^-1, tau = tau_range[i], psi = 0.656,
                       ...
                       phi = d_phi, kappa = d_kappa, alpha = d_alpha)
        ...
        if((dist[1] <= epsilon_dist[g]) && (dist[2] <= epsilon_foodH[g]) && (dist[3] <= epsilon_AMRH[g]) && 
          ...
          # Store results
          res.new[i,]<-c(d_betaAA, d_phi, d_kappa, d_alpha, d_betaHD, d_betaHH, d_betaHI_EU, d_imp_nEU, d_propres_impnEU) 
          dist_data[i,] <- dist
          print(res.new[i,])
          ...
          if(g==1){
            ...
            w1<-prod(c(sapply(c(1:3,5:9), function(b) dunif(res.new[i,b], min=lm.low[b], max=lm.upp[b])),
    ...
    colnames(res.new) <- c("betaAA", "phi", "kappa", "alpha", "betaHD", "betaHH","betaHI_EU", "imp_nEU", "propres_impnEU")
    write.csv(res.new, file = paste("complexmodel_ABC_SMC_gen_amp_nozeta_",g,".csv",sep=""), row.names=FALSE)
    ####
  }
  return(N_ITER_list)
}
...
lm.upp <- c(0.25, 2, 500, 1, 0.025, 0.1, 0.0025, 1, 1)
...
epsilon_dist <-  c(2, 1.75, 1.5, 1.25, 1, 0.9, 0.8, 0.75)
epsilon_foodH <- c(3.26, 3.26*0.75, 3.26*0.6, 3.26*0.5, 3.26*0.25, 3.26*0.2, 3.26*0.15, 3.26*0.1, 3.26*0.05)
epsilon_AMRH <-  c(0.185, 0.185*0.75, 0.185*0.6, 0.185*0.5, 0.185*0.25, 0.185*0.2, 0.185*0.15, 0.185*0.1, 0.185*0.05)
epsilon_foodA <- c(0.017173052, 0.017173052*0.75, 0.017173052*0.6, 0.017173052*0.5, 0.017173052*0.25, 0.017173052*0.2, 0.017173052*0.15, 0.017173052*0.1, 0.017173052*0.05)
epsilon_AMRA <-  c(0.3333333, 0.3333333*0.75, 0.3333333*0.6, 0.3333333*0.5, 0.3333333*0.25, 0.3333333*0.2, 0.3333333*0.15, 0.3333333*0.1, 0.3333333*0.05)
  ...
  plot_analysis[[j]] <- local({
    ...
    for (i in 1:length(parmtau)) {
      ...
      parms = c(ra = 60^-1, rh = (5.5^-1), ua = 240^-1, uh = 28835^-1, tau = parmtau[i], psi = 0.656,
                ...
                betaAA = 4.909260e-02 , betaHH = 3.411968e-02, betaHD =  3.745730e-03,
                betaHI_EU =  6.571886e-04, 
                
                imp_nEU = 4.862380e-01, propres_impnEU =  2.086881e-01,
                phi =  5.800748e-01, kappa =  7.017112e+01, alpha =  2.184930e-01)
