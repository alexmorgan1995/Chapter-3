...
country_data_gen <- country_data_gen[country_data_gen$num_test_tet >= 10,]
...
tet_post$gen <- as.vector(sapply(1:length(list.files(path = "C:/Users/amorg/Documents/PhD/Chapter_3/Models/fit_data", pattern = "^complexmodel_ABC_SMC_gen_tet.*?\\.csv")), 
...
for(i in 1:length(names)){ 
  tet_p_list[[i]] <- ggplot(l_tet_post[[i]], aes(x=value, fill= gen)) + geom_density(alpha=.5) + 
...
pairs_tet <- GGally::ggpairs(tet_post[-12], lower=list(continuous=my_fn)) + theme_bw()