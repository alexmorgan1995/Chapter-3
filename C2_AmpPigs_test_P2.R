...
ABC_algorithm <- function(N, G, sum.stats, distanceABC, fitmodel, tau_range, init.state, data)  {
  N_ITER_list <- list()
  fit_parms <- c("betaAA", "phi", "kappa", "alpha", "zeta", "betaHD", "betaHI_EU", "imp_nEU", "propres_impnEU")
  ...
  for(g in 1:G) {
    ...
    while(i <= N) {
      ...
      if(prior.non.zero(c(d_betaAA, d_phi, d_kappa, d_alpha, d_zeta, d_betaHD, d_betaHI_EU, d_imp_nEU, d_propres_impnEU))) {
        m <- 0
        thetaparm = c(ra = 60^-1, rh = (5.5^-1), ua = 240^-1, uh = 28835^-1, psi = 0.656,
                                     ...
                                     betaHD = d_betaHD, betaHI_EU = d_betaHI_EU, imp_nEU = d_imp_nEU, propres_impnEU = d_propres_impnEU,
        ...
        if((dist_mod[1] <= epsilon[["dist"]][g]) && (dist_mod[2] <= epsilon[["foodH"]][g]) && (dist_mod[3] <= epsilon[["AMRH"]][g]) && 
          ...
          res.new[i,]<- c(d_betaAA, d_phi, d_kappa, d_alpha, d_zeta, d_betaHD, d_betaHI_EU, d_imp_nEU, d_propres_impnEU) 
    ...
    colnames(res.new) <- c("betaAA", "phi", "kappa", "alpha", "zeta", "betaHD", "betaHI_EU", "imp_nEU", "propres_impnEU")
    write.csv(res.new, file = paste("C:/Users/amorg/Documents/PhD/Chapter_3/Models/Chapter-3/Model_Fit_Data/Part2/ABC_post_complex_amppigs_",g,".csv",sep=""), row.names=FALSE)
  }
  return(N_ITER_list)
}
