...
ABC_algorithm <- function(N, G, distanceABC, fitmodel, tau_range, init.state, data, epsilon, lm.low, lm.upp, thetaparm)  {
  ...
  for(g in 1:G) {
    ...
    write.csv(res.old, file = paste("//csce.datastore.ed.ac.uk/csce/biology/users/s1678248/PhD/Chapter_3/Models/Chapter-3/Part_1_Basic/no_import_fit/output/ABC_post_amppigs_noimp_",g,".csv",sep=""), row.names=FALSE)
    }
  return(out)
}
...
test <- ABC_algorithm(N = 1000,
                      ...
                      epsilon = list("dist" =  c(5, 4, 3.5, 3, 2.5, 2.25, 2, 1.8),
                                     "foodH" = c(0.593, 0.593*0.8, 0.593*0.6, 0.593*0.4, 0.593*0.3, 0.593*0.25, 0.593*0.2, 0.593*0.175),
                                     "AMRH" =  c(UK_hum_ampres, UK_hum_ampres*0.8, UK_hum_ampres*0.6, UK_hum_ampres*0.4, UK_hum_ampres*0.3, UK_hum_ampres*0.25, UK_hum_ampres*0.2, UK_hum_ampres*0.175),
                                     "foodA" = c(UK_cont, UK_cont*0.8, UK_cont*0.6, UK_cont*0.4, UK_cont*0.3, UK_cont*0.25, UK_cont*0.2, UK_cont*0.175),
                                     "AMRA" =  c(UK_amp_res, UK_amp_res*0.8, UK_amp_res*0.6, UK_amp_res*0.4, UK_amp_res*0.3, UK_amp_res*0.25, UK_amp_res*0.2, UK_amp_res*0.175)), 
                      lm.low = c(0, 0, 0, 0, 0, 0), 
                      lm.upp = c(0.02, 0.01, 30, 1, 0.025, 0.01), 
                      thetaparm = c(ra = 60^-1, rh = (5.5^-1), ua = 240^-1, uh = 28835^-1, psi = 1,
                                      fracimp = 0, propres_imp = 0, eta = 0.11016))

stopCluster(cl)

saveRDS(test, file = "//csce.datastore.ed.ac.uk/csce/biology/users/s1678248/PhD/Chapter_3/Models/Chapter-3/Part_1_Basic/no_import_fit/output/dist_amppig_part1_list.rds")

# Showing Posterior Dists -------------------------------------------------

post_dist_names <- grep("noimp",
                        list.files("//csce.datastore.ed.ac.uk/csce/biology/users/s1678248/PhD/Chapter_3/Models/Chapter-3/Part_1_Basic/no_import_fit/output"), value = TRUE)
