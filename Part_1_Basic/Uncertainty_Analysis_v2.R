...
for(z in 1:2) {
  ...
  for (i in 1:nrow(comb_parms)) {
    ...
    parms2 = c(ra = 60^-1, rh = (5.5^-1), ua = 240^-1, uh = 28835^-1, 
               ...
               psi = c(UK_food_usage, UK_food_pig_usage)[z], 
  ...
  plot1 <- ggplot(tauoutput, aes(FracImp, Propres_Imp, z = DecRes)) + metR::geom_contour_fill(color = "black", size = 0.1)  + 
    labs(x = bquote("Fraction of Imports Contaminated (Frac"["Imp"]~")"), y = bquote("Fraction of Contaminated Imports Resistant (PropRes"["Imp"]~")"), 
         title = paste0(c("Baseline","Pig")[z], " Import Scenario"), fill = "% Decrease in Res") +
  ...
  plot2 <- ggplot(tauoutput, aes(FracImp, Propres_Imp, z = IncFBD)) + metR::geom_contour_fill(color = "black", size = 0.1)  + 
    labs(x = bquote("Fraction of Imports Contaminated (Frac"["Imp"]~")"), y = bquote("Fraction of Contaminated Imports Resistant (PropRes"["Imp"]~")"), 
         title = paste0(c("Baseline","Pig")[z], " Import Scenario"), fill = "% Increase in FBD") +
    scale_y_continuous(expand = c(0,0)) + scale_x_continuous(expand = c(0, 0)) + theme_bw() +
    theme(legend.position = "bottom", legend.title = element_text(size=14), legend.text=element_text(size=12),  axis.text=element_text(size=14),
          axis.title.y=element_text(size=14),axis.title.x = element_text(size=14),  
          plot.title = element_text(size = 15, vjust = 3, hjust = 0.1, face = "bold"),
...
max_gen <- max(tauoutput$DecRes)
min_gen <- min(tauoutput$DecRes)
max_pig <- max(tauoutput1$DecRes)
min_pig <- min(tauoutput1$DecRes)

scale_low <- min(c(min_gen, min_pig))/ max(c(min_gen, min_pig))
scale_high <- max(c(max_gen, max_pig))/ min(c(max_gen, max_pig))
  
breaks1 <- seq(0.05, 0.2, by = 0.01)
...
heat_gen_fbd <- ggplot(tauoutput, aes(FracImp, Propres_Imp, z = IncFBD)) + metR::geom_contour_fill(breaks = breaks1, color = "black", size = 0.1)  + 
  ...
  scale_fill_viridis_b(breaks = breaks1, direction = 1, begin = 0.1, end = 0.9) + 
  scale_y_continuous(expand = c(0,0)) + scale_x_continuous(expand = c(0, 0)) + theme_bw() +
  theme(legend.position = "bottom", legend.title = element_text(size=14), legend.text=element_text(size=12),  axis.text=element_text(size=14),
        axis.title.y=element_text(size=14),axis.title.x = element_text(size=14),  
        plot.title = element_text(size = 15, vjust = 3, hjust = 0.1, face = "bold"),
...
heat_pig_fbd <- ggplot(tauoutput1, aes(FracImp, Propres_Imp, z = IncFBD)) + metR::geom_contour_fill(breaks = breaks1, color = "black", size = 0.1)  + 
  ...
  scale_fill_viridis_b(breaks = breaks1, direction =  1, begin = 0.1*scale_low, end = 0.9*scale_high) + 
  scale_y_continuous(expand = c(0,0)) + scale_x_continuous(expand = c(0, 0)) + theme_bw() +
  theme(legend.position = "bottom", legend.title = element_text(size=14), legend.text=element_text(size=12),  axis.text=element_text(size=14),
        axis.title.y=element_text(size=14),axis.title.x = element_text(size=14),  
        plot.title = element_text(size = 15, vjust = 3, hjust = 0.1, face = "bold"),
...
comb_heat <- ggarrange(heat_gen_fbd, heat_pig_fbd, ncol = 1, nrow = 2, labels = c("A", "B"), 
                       font.label = list(size = 20), vjust = 1.2, common.legend = TRUE, legend = "bottom")

ggsave(comb_heat, filename = "heat_comb_fbd.png", dpi = 300, type = "cairo", width = 8, height = 10, units = "in", 
       path = "C:/Users/amorg/Documents/PhD/Chapter_3/Figures/New_Figures")
