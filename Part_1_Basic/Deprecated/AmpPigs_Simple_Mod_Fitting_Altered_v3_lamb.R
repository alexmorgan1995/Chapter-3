...
ABC_algorithm <- function(N, G, distanceABC, fitmodel, tau_range, init.state, data, epsilon, lm.low, lm.upp, thetaparm)  {
  ...
  for(g in 1:G) {
    ...
    write.csv(res.old, file = paste("C:/Users/amorg/Documents/PhD/Chapter_3/Models/Chapter-3/Model_Fit_Data/Part1/ABC_post_amppigs_lamb_",g,".csv",sep=""), row.names=FALSE)
    }
  return(out)
}
...
test <- ABC_algorithm(N = 250,
                      ...
                      thetaparm = c(ra = 60^-1, rh = (5.5^-1), ua = 240^-1, uh = 28835^-1, psi = UK_food_lamb_usage,
...
post_dist_names <- grep("ABC_post_amppigs_lamb_",
