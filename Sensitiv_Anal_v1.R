...
parmdetails <- data.frame("parms" = c("fracimp", "propres_imp" ,"psi","betaAA" ,"betaHI","betaHD" ,"betaHH" ,"phi" ,"theta",
                                      "alpha", "zeta", "rh", "ra" ,"uh" , "ua" ),
                          "lbound" = c(0, 0, 0, 0.0029, 0.000001, 0.000001, 0.000001,  0.00131, 0.113,
                                       0, 0.00497, 55^-1, 600^-1, 288350^-1, 2400^-1),
                          "ubound" = c(1, 1, 1, 0.29, 0.0001, 0.0001, 0.0001, 0.131, 11.3,
...
ode_function_relfbd <- function(x) {
  ...
  for(z in 1:nrow(x)) {
    ...
    parms <- c(ra = x[z,1], rh = x[z,2], ua = x[z,3], uh = x[z,4], betaAA = x[z,5], betaHH = x[z,6], 
               betaHI = x[z,7], betaHD = x[z,8], phi = x[z,9], theta = x[z,10], alpha = x[z,11], zeta = x[z,12], psi = x[z,13], fracimp = x[z,14], propres_imp = x[z,15])
...
factors <- c("ra","rh", "ua", "uh", "betaAA", "betaHH", "betaHI", "betaHD","phi", "theta", "alpha","zeta" ,"psi" , "fracimp","propres_imp")
