...
MAP_parms <- data.frame("parms" = colnames(amp_post)[-11], "mean" = colMeans(amp_post[amp_post$gen == tail(unique(amp_post$gen), 1),][-11]))
...
parms = c(ra = 60^-1, rh = (5.5^-1), ua = 240^-1, uh = 28835^-1, tau = 0, psi = 0.656,
          ...
          betaAA = MAP_parms[which(MAP_parms == "betaAA"),2], MAP_parms[which(MAP_parms == "betaHH"),2], betaHD =  MAP_parms[which(MAP_parms == "betaHD"),2],
...
for(j in 1:length(names(amp_runif))){
  amp_uncert[[j]] <- local ({
    ...
    for(x in 1:length(amp_runif[[j]])) {
      dump_data <- vector() 
      parms2 <- parms
      parms2[names(amp_runif)[x]] <- amp_runif[[j]][[i]]
